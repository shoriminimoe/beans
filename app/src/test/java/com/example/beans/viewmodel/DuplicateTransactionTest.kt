...
class DuplicateTransactionTest {
    ...
    @Test
    fun `duplicate transaction creates new transaction with different id`() = runTest(testDispatcher) {
        ...
        assertEquals(original.postings.size, viewModel.transactions.value[1].postings.size)
    }

    @Test
    fun `duplicate transaction preserves all fields except id`() = runTest(testDispatcher) {
        viewModel.loadTransactions()
        advanceUntilIdle()
    ...
    @Test
    fun `duplicate transaction persists to file`() = runTest(testDispatcher) {
        viewModel.loadTransactions()
        advanceUntilIdle()
